...
const size_t MAX_QUERY_LENGTH = 2000;
...
private:
   ...
   */
  static void set_response_phrase(service::Phrase& resp_phrase,
  ...
  std::pair<QueryNormalizer::Options, SearchOptions> to_options(
    ...
    s_options.max_phrase_count = request.max_phrases();
    s_options.max_phrase_frequency = max_freq;
    s_options.phrase_length_max = max_len;
    s_options.phrase_length_min = min_len;
    // TODO: Replace these with configurable values
    s_options.pruning_high = 160000;
    s_options.pruning_low = 130000;

    QueryNormalizer::Options n_options;
    n_options.max_length = max_len;
    n_options.min_length = min_len;
    // TODO: Replace these with configurable values
    n_options.max_norm_queries = 10000;
    n_options.max_regex_matches = 100;
    n_options.max_regex_time = std::chrono::milliseconds(20);
  ...
  static std::unique_ptr<std::set<ref_>> merge_phrase_refs(
    ...
    for (const auto& ref_item : raw_references) {
      ...
      for (const auto& raw_ref : ref_item.result->refs()) {
        ref_ ref{ query, Phrase::Id(len, raw_ref.id()), raw_ref.freq() };
        set->insert(std::move(ref));
      }
    }

    return set;
  }
  std::unique_ptr<SearchResult> merge_raw_response_(
      const SearchOptions& options, const RawResult& raw_result) {
    auto search_result = std::make_unique<SearchResult>();
    util::vec_append(search_result->unknown_words(),
                     raw_result.unknown_words());
  ...
  std::unique_ptr<RawResult> process_norm_queries_(
      const SearchOptions& options, const std::vector<NormQuery>& normQueries) {
   ...
   */
  static std::string norm_query_to_key(const NormQuery& query) {
    std::string out;
  ...
  static bool is_prunable_from(const SearchOptions& superset,
  ...
  static std::shared_ptr<const RawRefResult> prune(
      const RawRefResult& result, const SearchOptions& options) {
  ...
  std::shared_ptr<const RawRefResult> process_wildcard_query_(
    ...
    std::shared_ptr<const RawRefResult> final_result;
    ...
    if (cached_result && cached_result->options == options) {
      ...
      // small result sets.
      final_result = prune(*cached_result->result, options);
    } else {
      // can't serve from cache
      final_result = query_processor_.process(options, query);
      if (cached_result &&
          !final_result->disjoint_with(*cached_result->result)) {
